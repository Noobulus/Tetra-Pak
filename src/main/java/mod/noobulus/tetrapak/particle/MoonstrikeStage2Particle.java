...
@OnlyIn(Dist.CLIENT)
public class MoonstrikeStage2Particle extends SpriteTexturedParticle {

    // this is pretty much exactly the same as the vanilla crit particle since i want it to behave the same
    protected MoonstrikeStage2Particle(ClientWorld worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn, double ySpeedIn, double zSpeedIn) {
    ...
    public static class Factory implements IParticleFactory<BasicParticleType> {
        ...
        public Particle makeParticle(BasicParticleType typeIn, ClientWorld worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn, double ySpeedIn, double zSpeedIn) {
            MoonstrikeStage2Particle moonstrikeStage2Particle = new MoonstrikeStage2Particle(worldIn, xCoordIn, yCoordIn, zCoordIn, xSpeedIn, ySpeedIn, zSpeedIn);
            moonstrikeStage2Particle.setColor(1.0f, 1.0f, 1.0f);
            moonstrikeStage2Particle.selectSpriteRandomly(this.spriteSet);
            return moonstrikeStage2Particle;
        }
    }
}
