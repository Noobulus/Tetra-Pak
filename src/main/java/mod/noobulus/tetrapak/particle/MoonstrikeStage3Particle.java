...
@OnlyIn(Dist.CLIENT)
public class MoonstrikeStage3Particle extends SpriteTexturedParticle {

    // this is pretty much exactly the same as the vanilla crit particle since i want it to behave the same
    protected MoonstrikeStage3Particle(ClientWorld worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn, double ySpeedIn, double zSpeedIn) {
    ...
    public static class Factory implements IParticleFactory<BasicParticleType> {
        ...
        public Particle makeParticle(BasicParticleType typeIn, ClientWorld worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn, double ySpeedIn, double zSpeedIn) {
            MoonstrikeStage3Particle moonstrikeStage3Particle = new MoonstrikeStage3Particle(worldIn, xCoordIn, yCoordIn, zCoordIn, xSpeedIn, ySpeedIn, zSpeedIn);
            moonstrikeStage3Particle.setColor(1.0f, 1.0f, 1.0f);
            moonstrikeStage3Particle.selectSpriteRandomly(this.spriteSet);
            return moonstrikeStage3Particle;
        }
    }
}
