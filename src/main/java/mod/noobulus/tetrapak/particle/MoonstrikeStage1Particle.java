...
@OnlyIn(Dist.CLIENT)
public class MoonstrikeStage1Particle extends SpriteTexturedParticle {

    // this is pretty much exactly the same as the vanilla crit particle since i want it to behave the same
    protected MoonstrikeStage1Particle(ClientWorld worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn, double ySpeedIn, double zSpeedIn) {
    ...
    public static class Factory implements IParticleFactory<BasicParticleType> {
        ...
        public Particle makeParticle(BasicParticleType typeIn, ClientWorld worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn, double ySpeedIn, double zSpeedIn) {
            MoonstrikeStage1Particle moonstrikeStage1Particle = new MoonstrikeStage1Particle(worldIn, xCoordIn, yCoordIn, zCoordIn, xSpeedIn, ySpeedIn, zSpeedIn);
            moonstrikeStage1Particle.setColor(1.0f, 1.0f, 1.0f);
            moonstrikeStage1Particle.selectSpriteRandomly(this.spriteSet);
            return moonstrikeStage1Particle;
        }
    }
}
